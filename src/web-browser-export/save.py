...
def _print_sqlite_db_to_stdout(pth: Path) -> None:
    ...
    if click.get_text_stream("stdout").isatty() and not force:
        logger.error(
            "stdout is a TTY, not printing database to stdout. Pipe to something else (e.g. web_browser_export save ... > db.sqlite, web_browser_export save ... | gzip --best > db.sqlite.gz) or set BROWSEREXPORT_FORCE=1 to print to stdout"
        )
        return
