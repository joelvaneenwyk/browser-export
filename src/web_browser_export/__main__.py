...
# target for python3 -m web_browser_export and console_script using click
@click.group(
...
def _wrap_browserexport_cli_errors() -> Iterator[None]:
    try:
        ...
        click.echo(
            f"{click.style('Error:', 'red')} {e}, run as 'web_browser_export --debug' for more info"
        )
        exit(1)
...
@cli.command(
    epilog=(
        "For a list of all browsers, run 'LIST_BROWSERS=1 web_browser_export save --help'"
...
def _handle_merge(dbs: List[str], *, json: bool, stream: bool) -> None:
    ...
    with _wrap_browserexport_cli_errors():
        for db in dbs:
            ...
            # e.g. `web_browser_export merge <(web_browser_export save -b chrome -t -)`
...
def merge(sqlite_db: Sequence[str], json: bool, stream: bool) -> None:
    ...
    web_browser_export merge ~/data/firefox/*.sqlite
...
if __name__ == "__main__":
    cli(prog_name="web_browser_export")
